...
const query = groq`*[_type == 'surface' && slug.current == $slug][0]{
  title,
  painting,
  detailone,
  detailtwo,
  ...
  "next": *[_type == 'surface' && ^._createdAt < _createdAt] | order(_createdAt asc)[0]{ 
      "slug": slug.current,title,order
  },
  "previous": *[_type == 'surface' && ^._createdAt > _createdAt] | order(_createdAt asc)[-1]{ 
      "slug": slug.current, title,order
  },
}`
...
export const getStaticPaths = async () => {
  const paths = await client.fetch(groq`*[_type == "surface" && defined(slug.current)][].slug.current`)
  return {
